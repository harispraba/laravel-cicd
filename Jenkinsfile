            ...
            steps {
                script {
                    withCredentials([
                        ...
                        sh "scp -v -o StrictHostKeyChecking=no -i /root/.ssh/id_rsa docker-compose.yml $SSH_USER@$SERVER:/opt/deployment-manifests/docker-compose.yml"
                        // Execute the Docker Compose command on the remote server
                        sh "ssh -v -o StrictHostKeyChecking=no -i /root/.ssh/id_rsa $SSH_USER@$SERVER 'docker-compose -f /opt/deployment-manifests/docker-compose.yml up -d'"
                    }
                }
            }
        }
