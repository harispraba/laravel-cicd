            ...
            steps {
                script {
                    withCredentials([
                        ...
                        sh "ssh -v -o StrictHostKeyChecking=no -i ${SSH_KEY} $SSH_USER@$SERVER 'sudo rm -f /opt/deployment-manifests/docker-compose.yml'"
                        sh "scp -v -o StrictHostKeyChecking=no -i ${SSH_KEY} docker-compose.yml $SSH_USER@$SERVER:/opt/deployment-manifests/docker-compose.yml"
                        sh "ssh -v -o StrictHostKeyChecking=no -i ${SSH_KEY} $SSH_USER@$SERVER 'echo $DOCKER_PASSWORD | sudo docker login ${DOCKER_REGISTRY} -u $DOCKER_USER --password-stdin'"
                        sh "ssh -v -o StrictHostKeyChecking=no -i ${SSH_KEY} $SSH_USER@$SERVER 'sudo docker compose -f /opt/deployment-manifests/docker-compose.yml up -d'"
                    }
                }
            }
        }
